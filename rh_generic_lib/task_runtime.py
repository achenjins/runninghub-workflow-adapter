...
class TaskRuntimeMixin:
    ...
    async def _job_slot(self, task_id):
        def external_reservations():
            ...
            if journal.get(task_id)["status"] != "queued":
                return 0
            return sum(r["status"] in {"pending", "tracking_paused", "submitting", "unknown_submission"}
            ...
            async with self._job_slot(task_id):
                ...
                while True:
                    ...
                    try:
                        ...
                        if exc.status.startswith("CANCEL"):
                    ...
                    await journal.mark_success(task_id, consume_coins_from_result(result), outputs)
    ...
    async def _deliver_job_once(self, record, client):
        ...
        try:
            await self._append_result_to_llm_context(record["stream_id"],
                [{"type": "text", "data": f"RunningHub 任务 {record['task_id']} 已完成，可用 rh_context 选择结果继续修改"}],
                visible_text=f"[生成结果] {record['workflow']} 已完成（{record['task_id']}）")
            await self._trigger_llm_result_reply(record["stream_id"])
        except Exception:
    ...
    async def _cancel_task(self, task_id, stream_id):
        ...
        if status == "queued":
            ...
            message = "任务已结束"
        await self._notify_job(record, f"{task_id}：{message}")
