...
- ✅ 每个工作流可自由配置输入节点（提示词 / 参考图 / 音频 / 视频 / 参数）；需要传参考文件时会一步步引导上传，**可只传一部分**
- ✅ 支持 Standard / Plus / Ultra 三档设备
- ✅ 可选 LLM 提示词扩写（一句话帮你扩写成完整提示词）
- ✅ 三种触发方式：命令、自然语言、公开 API
...
- 只支持「只有提示词输入」的工作流（文生图 / 文生视频这种）。带参考图、参考音频、视频、可改参数的工作流请用 `/rh运行` 命令。
...
**Q：为什么我说话没触发生成？**
A：自然语言只支持「只有提示词输入」的工作流；带参考图 / 音频 / 视频 / 参数的工作流请用 /rh运行。另外新增 / 改名工作流后要重载插件（必要时重启 MaiBot）才会刷新可用列表。
