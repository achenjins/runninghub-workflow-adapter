...
> 💰 **先说最要紧的**：RunningHub 是付费平台，每跑一次任务都会消耗你的账户余额 / 积分。建议装好后先配置「访问控制」（见文末），限制谁能用、每小时最多跑几次，避免被群里的人刷爆余额。
...
1. [这个插件能做什么](#这个插件能做什么)
2. [装好之后：三步跑通第一张图](#装好之后三步跑通第一张图)
3. [可选：开启提示词扩写](#可选开启提示词扩写)
4. [识别之后：整理你的输入节点](#识别之后整理你的输入节点)
5. [常用命令速查表](#常用命令速查表)
6. [小白必懂的几个概念](#小白必懂的几个概念)
7. [完整配置项参考](#完整配置项参考)
8. [进阶玩法](#进阶玩法)
9. [常见问题 FAQ](#常见问题-faq)
...
- ✅ 配置**多个**工作流，每个工作流可自定义名称、设备类型、国内 / 国外区域
- ✅ 每个工作流可自由配置输入节点：提示词 / 参考图 / 参考音频 / 参考视频 / 可修改参数 / 固定参数
- ✅ 需要传参考图、参考音频、视频时，会一步步引导你上传，**可以只传一部分**
- ✅ 支持 Standard / Plus / Ultra 三档设备
- ✅ 可选 LLM 提示词扩写（一句话帮你扩写成完整提示词）
- ✅ 命令、LLM 工具、公开 API 三种触发方式
- ✅ 结果自动发送；可选「发完自动撤回」（仅 NapCat / QQ 适配器生效）
...
保存后，再发 /跑图，提示词就会先自动扩写再出图了。

**默认调用哪个模型**：扩写用的是 MaiBot 里 **utils 槽位**配置的模型（也就是你给 MaiBot 配的通用快模型）。想换更强或更快的，可在插件配置页「提示词扩写 → 模型槽位」改成 replyer（主回复模型，更强）或 planner（规划快模型）。扩写完成后，结果会自动填入工作流里配置为「提示词」（prompt）的那个节点，再提交出图。

---

## 识别之后：整理你的输入节点

用 /识别工作流 导入后，插件会自动列出一串「输入节点」。这些节点是插件按常见规则猜出来的，不一定 100% 符合你的需求，建议看一眼再正式用。
...
| 节点类型 | 建议 |
|---------|------|
| **提示词**（prompt） | **保留**。这是你每次输入描述文字的核心位置；**整个工作流仅支持一个提示词输入节点** |
...
> 如果工作流有多个提示词输入（如正 / 负提示词），**只保留一个「提示词」（prompt）节点**，其余提示词请改成「可编辑配置」（text）——因为扩写功能只会把结果适配传入「提示词」类型的那个节点。
...
| 字段 | 填什么 |
|------|--------|
| 节点 ID | 工作流里那个节点的编号（一串数字） |
| 字段名 | 要填内容的位置，如 prompt / image / text / width |
| 输入内容 | 默认值；留空 = 运行时由用户提供 |
| 节点类型 | prompt / text / default / image / audio / video，或「自动推断」 |
| 输入说明 | 提示用户时的中文说明，如「角色参考图」 |

**节点 ID 和字段名从哪看**：打开 RunningHub 网站上的工作流，每个节点都有一个编号（就是「节点 ID」），节点上要填内容的输入框名就是「字段名」。实在找不到，就先用 /详细识别工作流 自动填好，再照着改。
...
| 命令 | 作用 | 例子 |
|------|------|------|
| **/跑图 工作流名 描述** | 运行工作流生成图片 / 视频 | /跑图 动漫生图 一只猫 |
| **/工作流** | 列出已配置的工作流 | /工作流 |
| **/识别国外工作流 ID 名称** | 导入 runninghub.ai 上的工作流（关键节点） | /识别国外工作流 2087... 动漫生图 |
| **/识别国内工作流 ID 名称** | 导入 runninghub.cn 上的工作流（关键节点） | /识别国内工作流 2087... 动漫生图 |
| **/详细识别国外工作流 ID 名称** | 导入国外工作流，并识别**全部**参数 | /详细识别国外工作流 2087... 动漫生图 |
| **/详细识别国内工作流 ID 名称** | 导入国内工作流，并识别**全部**参数 | /详细识别国内工作流 2087... 动漫生图 |
...
| 类型 | 一句话解释 | 运行时行为 |
|------|-----------|-----------|
| **提示词**（prompt） | 你要生成什么 | 接收 /跑图 后面的描述文字；**整个工作流仅支持一个提示词输入节点** |
...
> 这些都不用手动懂原理——用 /识别工作流 导入后基本就自动填好了，需要微调再去插件配置页改。
...
> ⚠️ 默认**全部放行**——任何能跟你机器人聊天的人都能触发任务（花你的钱）。建议至少填一个：要么加白名单，要么设个频率上限。
...
**Q：结果图片想让它自己删掉？**
A：这是「自动撤回」功能，仅 NapCat / QQ 适配器可用，在「自动撤回」里开启并设秒数。
