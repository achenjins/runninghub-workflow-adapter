...
1. **删掉多余节点**：进入 MaiBot 插件配置页 → 工作流列表 → 拉到最下找到「文生图」工作流，把识别出来的输入节点**只保留「提示词」节点（海外工作流的提示词节点是 353 号）**，其余节点全部删除。
...
进阶玩法：也可以在 MaiBot 的人格提示词里加入规则，让 AI 自己模拟场景并生成“当前”的图片——例如用户聊到咖啡厅、图书馆时，AI 可以发一张自己在咖啡厅 / 图书馆等场景的图。
